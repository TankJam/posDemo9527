...
// Blockchain 声明一个区块链，类型是切片，也就是将区块放在这个切片里面
var Blockchain []Block

// generateBlock 出块函数，也就是当随机抽中哪个账户后，该账户就会作为该区块的参数，生成一个最新的区块
...
// calculateHash SHA256算法计算当前区块的hash值
...
// calculateBlockHash 将区块字段拼接，为生成hash函数做准备
...
func main() {
	...
	// 通过下边的for循环，按照两个节点的持币数，分别将他们的地址赋值到之前定义好的账户地址数组中
